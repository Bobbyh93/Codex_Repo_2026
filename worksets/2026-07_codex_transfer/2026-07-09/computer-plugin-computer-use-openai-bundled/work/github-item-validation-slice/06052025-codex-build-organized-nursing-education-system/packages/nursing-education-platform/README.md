...
The deterministic tests verify the default four-item adaptive path, ability updates, session export, weak-concept detection, readiness gates, blueprint coverage matrix, cross-session exposure control, stopping rules, local fallback storage, server-backed JSON attempt persistence, and educator item import validation.
...
- Import Validation: checks educator-authored item JSON against the metadata contract before it can be treated as pilot-ready content.
...
- `public/item-validation.js`: item-bank and import-payload metadata contract validation.
...
- `test/item-validation.test.mjs`: item contract and import validation checks.
...
Attempt history now persists to a local JSON file when served through `npm run dev`, and falls back to browser local storage if the API is unavailable. Item import validation is a metadata readiness gate, not a secure authoring workflow. Production learner records still need authenticated database persistence, audit trails, secure delivery, and institution-ready item exposure analytics.

## Next Engineering Work

- Add authenticated educator item authoring, import persistence, and review workflow.
